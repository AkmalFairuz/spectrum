...
type Animation interface {
	...
	Play(conn Conn, serverGameData minecraft.GameData)
	// Clear stops the animation previously started on the connection using the new server's GameData.
	Clear(conn Conn, serverGameData minecraft.GameData)
}
...
func (NopAnimation) Play(_ Conn, _ minecraft.GameData)  {}
func (NopAnimation) Clear(_ Conn, _ minecraft.GameData) {}

type cameraAnimation struct {
	synced atomic.Bool
}

func (animation *cameraAnimation) Sync(conn Conn) {
	if animation.synced.CompareAndSwap(false, true) {
		_ = conn.WritePacketToClient(&packet.CameraPresets{
